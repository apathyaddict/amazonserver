...
function getCategoryApi(req, res) {
    ...
    let ASIN = req.query.asin;
      ...
      api
        ...
        .then((data) => {
            let getBrowseNodesResponse = 
            ProductAdvertisingAPIv1.GetBrowseNodesResponse.constructFromObject(data);
            console.log('Complete Response: \n' + JSON.stringify(getBrowseNodesResponse, null, 1));
       
          if (getBrowseNodesResponse["Errors"] !== undefined) {
            res.status(400).json(getBrowseNodesResponse["Errors"]);
          } else {
            res.status(200).json(getBrowseNodesResponse);
          }
        })
        .catch((error) => {
          console.log("Error calling PA-API 5.0!");
